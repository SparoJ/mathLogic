package com.sparo.leetcode.story;

/**
 * description:
 * Created by sdh on 2020-01-17
 */
public class Queen {
    ...
    public static boolean check(int k,int j){//判断节点是否合适
        // 只需要对当前列进行exclude 同行不可能有多个 queen
        System.out.print("checkRow->" + k + "checkCol->" + j + "\n");
        for(int i=0;i<8;i++){//检查行列冲突
            if(arry[i][j]==1){
                return false;
            }
        }
        //只需要对当前queen 所在位置的 左上和 右上exclude 因为左下和右下不可能有queen
        for(int i=k-1,m=j-1; i>=0 && m>=0; i--,m--){//检查左对角线
            System.out.print("lrow->" + i + "lcol->" + m + "\n");
            if(arry[i][m]==1){
                return false;
            }
        }
        for(int i=k-1,m=j+1; i>=0 && m<=7; i--,m++){//检查右对角线
            System.out.print("rrow->" + i + "rcol->" + m + "\n");
            if(arry[i][m]==1){
                return false;
            }
        }
